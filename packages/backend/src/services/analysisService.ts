...
export async function discoverSources(
  ...
  const { inserted, droppedCount } = await coverageRepo.addCoveragesUpToLimit(
    ...
    verified.map((c) => ({
      ...
      primaryCategory: resolveCategoryForCandidate(c),
    })),
    MAX_COVERAGES_PER_ANALYSIS
  )
  if (droppedCount > 0) {
...
/** `isAdmin` mirrors `listAnalyses`'s own gate (ticket 03) and ADR 0035's "public reads are
 *  bounded by COMPLETE status" precedent — extended here to the single-Analysis read, not just
 *  the list. A non-Admin caller (including unauthenticated) gets the exact same NotFoundError a
 *  genuinely missing id produces for any status other than COMPLETE, so a Draft/Pending/streaming
 *  Analysis's in-progress internals (source list, per-source extraction state, partial synthesis)
 *  are never observable via this endpoint — ticket 52's public `/article/:id` route relies on this
...
export async function getAnalysisDetail(analysisId: string, isAdmin: boolean): Promise<AnalysisDetail> {
  ...
  // Pure DB reads (ADR 0028): the Cross-Source Narrative is generated by the narrative.generate
