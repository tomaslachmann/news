...
> **Partially reopened by [ticket 07](07-thread-aggregate.md) / [ADR 0028](../../../docs/adr/0028-pg-boss-job-queue-adoption.md).** `Thread`'s `thread.recompute` has no synchronous fallback the way this ticket's P0-5/P2-22 fixes did, so pg-boss is adopted after all — scoped to `entity.extract`/`relation.link`, `narrative.generate` (superseding this ticket's ADR 0026 TTL fix), and `thread.recompute` only. Everything else this ticket declined (scraping/politeness, `story.match`, the SSE-streamed analysis flow) is unaffected — this amends the deferral, it doesn't overturn it.
