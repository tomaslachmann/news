...
- [x] `AnalysisPage.tsx`: entity mentions for the current Event link to their own entity pages, using the `key` already present on `StoryEntity`.
- [x] Entity page renders "no Wikidata link" / "no known aliases" gracefully when tickets 40/41 haven't shipped or haven't linked this particular entity yet — never a broken or missing section.
- [x] Public listing/homepage unchanged — this is an additive navigation surface only.
