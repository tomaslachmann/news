...
- [x] `packages/shared/src/index.ts`: mirror the enum as a matching TS union/label map (same
...
- [x] New per-source raw-category mapping table (e.g.
...
- [x] `packages/backend/src/services/rss.ts`: extend `RawFeedItem`/`parseRss2` to also capture each
      item's raw category value(s) (`item.categories` from the underlying `rss-parser` — currently
      discarded entirely).
- [x] Wherever a `CandidateArticle`/`Coverage` gets created from a parsed feed item: resolve
...
- [x] Story/Analysis read-time aggregate: a pure function (e.g. in `mappers/` or a service) that
...
- [x] `CONTEXT.md`: add a "Category" entry defining `ArticleCategory`, the per-Coverage/aggregate
      split, and the "no backfill" rule, so this new domain term doesn't drift.
- [x] Tests: mapping-table resolution (first-mappable-tag-wins, no-match → null, unknown source →
