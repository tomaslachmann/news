...
- [x] `services/entityAliases/index.ts` + `hooks.ts` (frontend), mirroring `services/ingestion/`'s
...
- [x] New page (e.g. `EntityAliasesPage.tsx`) with a candidates list: each row shows both entities'
...
- [x] Wired into the router: new route (e.g. `/admin/entity-aliases`) under the existing
  `AdminLayout`/`ProtectedRoute` wrapper in `App.tsx`, following `/admin/ingestion`'s exact shape.
- [x] Nav link added to `AdminChrome.tsx` alongside the existing "Kontrola sběru"/"Uživatelé" links.
- [x] Empty state when there are no candidates above the similarity threshold.
